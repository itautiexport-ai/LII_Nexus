...
exports.MySqlFactoryProductionEntryRepository = void 0;
...
`;
class MySqlFactoryProductionEntryRepository {
    async attachFiles(entries) {
        if (entries.length === 0)
            return entries;
        const ids = entries.map((e) => e.id);
        const placeholders = ids.map(() => "?").join(",");
        const [fileRows] = await connection_1.pool.query(`SELECT * FROM factory_production_entry_files WHERE entry_id IN (${placeholders})`, ids);
    ...
    async list(params) {
        ...
        if (params.from) {
            conditions.push("fpe.entry_date >= ?");
        ...
        const [rows] = await connection_1.pool.query(`${WITH_CONTEXT_SELECT} ${whereClause} ORDER BY fpe.entry_date DESC, fpe.created_at DESC LIMIT ? OFFSET ?`, [...values, params.pageSize, offset]);
        const [countRows] = await connection_1.pool.query(`SELECT COUNT(*) as total FROM factory_production_entries fpe ${whereClause}`, values);
    ...
    async findById(id) {
        const [rows] = await connection_1.pool.query("SELECT * FROM factory_production_entries WHERE id = ? AND deleted_at IS NULL", [id]);
        return rows[0] ? mapRow(rows[0]) : null;
    }
    async getWithContext(id) {
        const [rows] = await connection_1.pool.query(`${WITH_CONTEXT_SELECT} WHERE fpe.id = ? AND fpe.deleted_at IS NULL`, [id]);
        if (!rows[0])
            return null;
        const r = rows[0];
        const item = {
    ...
    async update(id, changes) {
        ...
        if (fields.length > 0) {
            values.push(id);
            await connection_1.pool.query(`UPDATE factory_production_entries SET ${fields.join(", ")} WHERE id = ?`, values);
        }
        return (await this.findById(id));
    }
    async approve(id, reviewedBy) {
        await connection_1.pool.query("UPDATE factory_production_entries SET status = 'approved', reviewed_by = ?, reviewed_at = NOW(), rejection_reason = NULL WHERE id = ?", [reviewedBy, id]);
        return (await this.findById(id));
    }
    async reject(id, reviewedBy, reason) {
        await connection_1.pool.query("UPDATE factory_production_entries SET status = 'rejected', reviewed_by = ?, reviewed_at = NOW(), rejection_reason = ? WHERE id = ?", [reviewedBy, reason, id]);
        return (await this.findById(id));
    }
    async softDelete(id) {
        await connection_1.pool.query("UPDATE factory_production_entries SET deleted_at = NOW() WHERE id = ?", [id]);
    }
    async addFile(entryId, kind, fileName, fileUrl, uploadedBy) {
        await connection_1.pool.query("INSERT INTO factory_production_entry_files (id, entry_id, kind, file_name, file_url, uploaded_by) VALUES (?, ?, ?, ?, ?, ?)", [(0, uuid_1.v4)(), entryId, kind, fileName, fileUrl, uploadedBy]);
    }
}
exports.MySqlFactoryProductionEntryRepository = MySqlFactoryProductionEntryRepository;
//# sourceMappingURL=MySqlFactoryProductionEntryRepository.js.map