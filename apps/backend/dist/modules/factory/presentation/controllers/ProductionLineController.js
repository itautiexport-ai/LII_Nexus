"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProductionLineController = void 0;
const ProductionLineService_1 = require("../../application/services/ProductionLineService");
const MySqlProductionLineRepository_1 = require("../../infrastructure/repositories/MySqlProductionLineRepository");
const apiResponse_1 = require("../../../../shared/utils/apiResponse");
const service = new ProductionLineService_1.ProductionLineService(new MySqlProductionLineRepository_1.MySqlProductionLineRepository());
exports.ProductionLineController = {
    async list(_req, res) {
        return (0, apiResponse_1.ok)(res, await service.list());
    },
    async create(req, res) {
        return (0, apiResponse_1.created)(res, await service.create(req.body, req.user.sub));
    },
    async update(req, res) {
        return (0, apiResponse_1.ok)(res, await service.update(req.params.id, req.body, req.user.sub));
    },
    async remove(req, res) {
        await service.remove(req.params.id, req.user.sub);
        return (0, apiResponse_1.ok)(res, { message: "Production line deleted." });
    },
};
//# sourceMappingURL=ProductionLineController.js.map