...
exports.FmsExecutionService = void 0;
const uuid_1 = require("uuid");
class FmsExecutionService {
    constructor(dbPool) {
        this.dbPool = dbPool;
    }
    async startFmsInstance(fmsManagerId, dto) {
        const instanceId = (0, uuid_1.v4)();
        // Create instance
        await this.dbPool.query("INSERT INTO fms_instances (id, fms_manager_id, reference_title, status, form_data, creator_id) VALUES (?, ?, ?, ?, ?, ?)", [instanceId, fmsManagerId, dto.referenceTitle, 'In Progress', dto.formData ? JSON.stringify(dto.formData) : null, dto.creatorId || null]);
        // Get steps for this manager
        const [steps] = await this.dbPool.query("SELECT id FROM fms_steps WHERE fms_id = ? ORDER BY sequence_order ASC", [fmsManagerId]);
        // Insert instance steps
        for (const step of steps) {
            await this.dbPool.query("INSERT INTO fms_instance_steps (id, instance_id, fms_step_id, status) VALUES (?, ?, ?, ?)", [(0, uuid_1.v4)(), instanceId, step.id, 'Pending']);
        }
        return { id: instanceId, message: "FMS instance started successfully" };
    }
    async getMyPendingTasks(employeeId) {
        const query = `
      SELECT 
    ...
    async getInstancesByManagerId(fmsManagerId) {
        const query = `
      SELECT 
        fi.id as instanceId,
        fi.reference_title as referenceTitle,
    ...
    async completeStep(employeeId, instanceStepId, dto) {
        // Check step details
    ...
    `, [instanceStepId]);
        if (rows.length === 0)
            throw new Error("Instance step not found");
        const step = rows[0];
        // Authorization check
        ...
        // Mark updated
        await this.dbPool.query("UPDATE fms_instance_steps SET status = ?, completed_by = ?, input_data = ?, completed_at = NOW() WHERE id = ?", [newStatus, employeeId, JSON.stringify(dto.inputData || {}), instanceStepId]);
      ...
      `, [step.instance_id, currentOrder, targetOrder]);
        ...
        const [pendingSteps] = await this.dbPool.query("SELECT id FROM fms_instance_steps WHERE instance_id = ? AND status = 'Pending'", [step.instance_id]);
...
exports.FmsExecutionService = FmsExecutionService;
//# sourceMappingURL=FmsExecutionService.js.map