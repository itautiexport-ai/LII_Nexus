...
exports.MySqlCrmRepository = void 0;
const uuid_1 = require("uuid");
const connection_1 = require("../../../../infrastructure/database/mysql/connection");
const Lead_1 = require("../../domain/entities/Lead");
...
class MySqlCrmRepository {
    async list(params) {
        ...
        if (params.assignedMerchantId) {
            conditions.push("cl.assigned_merchant_id = ?");
            values.push(params.assignedMerchantId);
        }
        if (params.status) {
        ...
        if (params.salesStage) {
            conditions.push("cl.sales_stage = ?");
            values.push(params.salesStage);
        }
        if (params.leadSource) {
            conditions.push("cl.lead_source = ?");
            values.push(params.leadSource);
        }
        if (params.leadCategory) {
            conditions.push("cl.lead_category = ?");
            values.push(params.leadCategory);
        }
        if (params.priority) {
            conditions.push("cl.priority = ?");
            values.push(params.priority);
        }
        if (params.overdueOnly) {
            conditions.push("cl.status = 'active' AND cl.next_follow_up_date < CURDATE()");
        }
        const whereClause = `WHERE ${conditions.join(" AND ")}`;
        const [rows] = await connection_1.pool.query(`${WITH_CONTEXT_SELECT} ${whereClause} ORDER BY cl.updated_at DESC LIMIT ? OFFSET ?`, [...values, params.pageSize, offset]);
        const [countRows] = await connection_1.pool.query(`SELECT COUNT(*) as total FROM crm_leads cl ${whereClause}`, values);
        return { items: rows.map(mapWithContext), total: countRows[0].total };
    }
    async findById(id) {
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_leads WHERE id = ? AND deleted_at IS NULL", [id]);
        return rows[0] ? mapLead(rows[0]) : null;
    }
    async getWithContext(id) {
        const [rows] = await connection_1.pool.query(`${WITH_CONTEXT_SELECT} WHERE cl.id = ? AND cl.deleted_at IS NULL`, [id]);
        return rows[0] ? mapWithContext(rows[0]) : null;
    }
    async findByLeadCode(leadCode) {
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_leads WHERE lead_code = ? AND deleted_at IS NULL", [leadCode]);
        return rows[0] ? mapLead(rows[0]) : null;
    }
    async nextLeadCodeSequence() {
        const [rows] = await connection_1.pool.query("SELECT COUNT(*) as total FROM crm_leads");
    ...
    async update(id, changes) {
        ...
        values.push(id);
        await connection_1.pool.query(`UPDATE crm_leads SET ${fields.join(", ")} WHERE id = ?`, values);
        return (await this.findById(id));
    }
    async assign(id, merchantId, updatedBy) {
        await connection_1.pool.query("UPDATE crm_leads SET assigned_merchant_id = ?, updated_by = ? WHERE id = ?", [merchantId, updatedBy, id]);
        return (await this.findById(id));
    }
    async softDelete(id) {
        await connection_1.pool.query("UPDATE crm_leads SET deleted_at = NOW() WHERE id = ?", [id]);
    }
    async logFollowup(data) {
        const id = data.id || (0, uuid_1.v4)();
        await connection_1.pool.query("INSERT INTO crm_lead_followups (id, lead_id, due_date, remarks, next_action, logged_by) VALUES (?, ?, ?, ?, ?, ?)", [id, data.leadId, data.dueDate, data.remarks ?? null, data.nextAction ?? null, data.loggedBy]);
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_lead_followups WHERE id = ?", [id]);
        return mapFollowup(rows[0]);
    }
    async completeFollowup(id, remarks, loggedBy) {
        await connection_1.pool.query(`UPDATE crm_lead_followups
       SET completed_at = NOW(), on_time = (DATE(NOW()) <= due_date), remarks = COALESCE(?, remarks), logged_by = ?
       WHERE id = ?`, [remarks, loggedBy, id]);
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_lead_followups WHERE id = ?", [id]);
        return mapFollowup(rows[0]);
    }
    async getPendingFollowup(leadId) {
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_lead_followups WHERE lead_id = ? AND completed_at IS NULL ORDER BY due_date DESC LIMIT 1", [leadId]);
        return rows[0] ? mapFollowup(rows[0]) : null;
    }
    async listFollowupsForLead(leadId) {
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_lead_followups WHERE lead_id = ? ORDER BY created_at DESC", [leadId]);
        return rows.map(mapFollowup);
    }
    async addFile(leadId, fileName, fileUrl, uploadedBy) {
        const id = (0, uuid_1.v4)();
        await connection_1.pool.query("INSERT INTO crm_lead_files (id, lead_id, file_name, file_url, uploaded_by) VALUES (?, ?, ?, ?, ?)", [id, leadId, fileName, fileUrl, uploadedBy]);
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_lead_files WHERE id = ?", [id]);
        const r = rows[0];
    ...
    async listFilesForLead(leadId) {
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_lead_files WHERE lead_id = ?", [leadId]);
    ...
    async createQuotation(data) {
        const id = (0, uuid_1.v4)();
        await connection_1.pool.query("INSERT INTO crm_quotations (id, buyer_id, sku_code, product_name, product_image_url, status) VALUES (?, ?, ?, ?, ?, ?)", [id, data.buyerId, data.skuCode, data.productName, data.productImageUrl || null, data.status || 'draft']);
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_quotations WHERE id = ?", [id]);
        return rows[0];
    }
    async updateQuotationStatus(id, status) {
        await connection_1.pool.query("UPDATE crm_quotations SET status = ? WHERE id = ?", [status, id]);
    }
    async addQuotationQuote(data) {
        const id = (0, uuid_1.v4)();
        await connection_1.pool.query("INSERT INTO crm_quotation_quotes (id, quotation_id, quote_name, currency, price, notes) VALUES (?, ?, ?, ?, ?, ?)", [id, data.quotationId, data.quoteName, data.currency || 'USD', data.price, data.notes || null]);
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_quotation_quotes WHERE id = ?", [id]);
        return rows[0];
    }
    async listQuotationQuotes(quotationId) {
        const [rows] = await connection_1.pool.query("SELECT * FROM crm_quotation_quotes WHERE quotation_id = ? ORDER BY created_at ASC", [quotationId]);
        return rows;
    }
}
exports.MySqlCrmRepository = MySqlCrmRepository;
//# sourceMappingURL=MySqlCrmRepository.js.map