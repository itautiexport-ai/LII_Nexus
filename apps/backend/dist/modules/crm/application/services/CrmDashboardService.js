...
const MerchantMetricsService_1 = require("./MerchantMetricsService");
...
class CrmDashboardService {
    ...
    async getCeoDashboard() {
       ...
       FROM crm_leads WHERE deleted_at IS NULL GROUP BY status`);
        const [categoryRows] = await connection_1.pool.query(`SELECT lead_category, COUNT(*) as count FROM crm_leads WHERE deleted_at IS NULL AND status = 'active' GROUP BY lead_category`);
        const [overdueRows] = await connection_1.pool.query(`SELECT COUNT(*) as count FROM crm_leads WHERE deleted_at IS NULL AND status = 'active' AND next_follow_up_date < CURDATE()`);
        return {
    ...
    async getLeadSourceDashboard() {
        const [rows] = await connection_1.pool.query(`SELECT lead_source,
         COUNT(*) as total,
    ...
    async getFollowUpDelayDashboard() {
        const [overdueRows] = await connection_1.pool.query(`SELECT cl.id, cl.lead_code, cl.contact_name, e.full_name as merchant_name, cl.next_follow_up_date,
       ...
       ORDER BY days_overdue DESC LIMIT 50`);
        const [byMerchantRows] = await connection_1.pool.query(`SELECT e.full_name as merchant_name,
    ...
    async getWonLostAnalysis() {
        const [rows] = await connection_1.pool.query(`SELECT lead_category, lead_source,
         SUM(CASE WHEN status = 'won' THEN 1 ELSE 0 END) as won,
         SUM(CASE WHEN status = 'lost' THEN 1 ELSE 0 END) as lost,
...
//# sourceMappingURL=CrmDashboardService.js.map