...
function getPriorityPoints(priority) {
    ...
    if (p === "high")
        return 3;
    if (p === "extremely high" || p === "urgent" || p === "critical")
        return 4;
    return 2; // default
}
function getRatingAndMultiplier(score) {
    if (score >= 9)
        return { rating: "Exceptional", multiplier: 1.25 };
    if (score >= 8)
        return { rating: "Excellent", multiplier: 1.10 };
    if (score >= 6)
        return { rating: "Good", multiplier: 1.00 };
    if (score >= 4)
        return { rating: "Average", multiplier: 0.70 };
    if (score >= 2)
        return { rating: "Weak", multiplier: 0.30 };
    return { rating: "Critical", multiplier: 0.00 };
}
class MisService {
    async generateReport(userId, period = "monthly") {
        const [[user]] = await connection_1.pool.query("SELECT * FROM users WHERE id = ?", [userId]);
        if (!user)
            throw new Error("User not found");
        const [[emp]] = await connection_1.pool.query("SELECT id FROM employees WHERE user_id = ?", [userId]);
        ...
        // 1. Delegation Tasks
        const [delTasks] = await connection_1.pool.query(`SELECT id, title, due_date, base_status, priority, completed_at 
       ...
       WHERE assigned_to = ? AND deleted_at IS NULL
        ...
        // 2. Checklist Tasks
        const [chkInstances] = await connection_1.pool.query(`SELECT ci.id, sc.task_name as title, sc.priority, cii.is_checked, cii.checked_at, ci.period_end 
       ...
       WHERE ci.employee_id = ?
        ...
        const [fmsRows] = await connection_1.pool.query(`SELECT ft.id, ft.base_status, ft.due_date, ft.completed_at, w.name as title 
       ...
       WHERE ft.assigned_to = ?
        ...
        const calculateModule = (totalDue, completed, onTime, totalCount, completedCount, pendingCount, runningCount, list) => {
        ...
        const [weightRows] = await connection_1.pool.query("SELECT * FROM module_weights LIMIT 1");
        ...
        if (/^\d{4}-W\d{2}$/.test(period) || period === 'weekly') {
            ...
            [hodRows] = await connection_1.pool.query(`SELECT score FROM hod_evaluations WHERE employee_id = ? AND evaluation_period = ?`, [empId, targetWeek]);
            [hrRows] = await connection_1.pool.query(`SELECT score, attendance_percentage FROM hr_evaluations WHERE employee_id = ? AND evaluation_period = ?`, [empId, targetWeek]);
        }
        else {
            [hodRows] = await connection_1.pool.query(`SELECT score FROM hod_evaluations WHERE employee_id = ? AND created_at >= ? AND created_at <= ?`, [empId, startStr, endStr]);
            [hrRows] = await connection_1.pool.query(`SELECT score, attendance_percentage FROM hr_evaluations WHERE employee_id = ? AND created_at >= ? AND created_at <= ?`, [empId, startStr, endStr]);
        }
        let hodScore = null;
        let hrScore = null;
        let attendancePercentage = null;
        let managerEvaluationScore = null;
        if (hodRows.length > 0) {
    ...
    async saveManagerEvaluation(userId, evaluatedBy, periodType, periodStart, periodEnd, data) {
        const [[emp]] = await connection_1.pool.query("SELECT id FROM employees WHERE user_id = ?", [userId]);
        const empId = emp ? emp.id : userId;
        const id = crypto_1.default.randomUUID();
        await connection_1.pool.query(`INSERT INTO manager_evaluations 
    ...
    async getCumulativeScores(period = "yearly") {
        const [employees] = await connection_1.pool.query("SELECT user_id FROM employees WHERE status = 'active'");
        const reports = [];
...
exports.MisService = MisService;
//# sourceMappingURL=ApgsService.js.map