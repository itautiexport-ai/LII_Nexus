...
exports.officeEmService = exports.OfficeEmService = void 0;
...
class OfficeEmService {
    async generateGapScoreReport(userId, period = "monthly") {
        ...
        const evaluateModule = (tasks, standardWeight) => {
        ...
        // 1. Delegation
        const [delTasks] = await connection_1.pool.query(`SELECT id, title as name, priority, due_date, base_status, completed_at
       ...
       WHERE assigned_to = ? AND deleted_at IS NULL
        ...
        // 2. Checklist
        const [chkRuns] = await connection_1.pool.query(`SELECT ci.id, sc.task_name as name, sc.priority, ci.period_end as due_date, 
       ...
       WHERE ci.employee_id = ?
        ...
        // 3. FMS
        const [fmsTasks] = await connection_1.pool.query(`SELECT ft.id, ws.name, 'medium' as priority, ft.due_date, ft.base_status, ft.completed_at
       ...
       WHERE ft.assigned_to = ?
        ...
        if (fmsScore.isActive)
            totalActiveWeight += fmsScore.standardWeight;
        if (checklistScore.isActive)
            totalActiveWeight += checklistScore.standardWeight;
        if (delegationScore.isActive)
            totalActiveWeight += delegationScore.standardWeight;
        if (totalActiveWeight > 0) {
            if (fmsScore.isActive)
                fmsScore.normalizedWeight = (fmsScore.standardWeight / totalActiveWeight) * 100;
            if (checklistScore.isActive)
                checklistScore.normalizedWeight = (checklistScore.standardWeight / totalActiveWeight) * 100;
            if (delegationScore.isActive)
                delegationScore.normalizedWeight = (delegationScore.standardWeight / totalActiveWeight) * 100;
        }
    ...
    async generateGapScoreList(period = "monthly") {
        const [users] = await connection_1.pool.query("SELECT id FROM users WHERE status = 'active' OR status = 'Active'");
        const reports = [];
...
exports.OfficeEmService = OfficeEmService;
exports.officeEmService = new OfficeEmService();
//# sourceMappingURL=OfficeEmService.js.map