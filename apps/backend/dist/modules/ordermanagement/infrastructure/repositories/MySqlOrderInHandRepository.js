...
exports.MySqlOrderInHandRepository = void 0;
const connection_1 = require("../../../../infrastructure/database/mysql/connection");
const uuid_1 = require("uuid");
class MySqlOrderInHandRepository {
    async findAll() {
        const [rows] = await connection_1.pool.query("SELECT * FROM orders_in_hand ORDER BY created_at DESC");
        return rows.map(this.mapToEntity);
    }
    async findById(id) {
        const [rows] = await connection_1.pool.query("SELECT * FROM orders_in_hand WHERE id = ?", [id]);
        if (!rows[0])
            return null;
    ...
    async generateOrderId() {
        ...
        const [rows] = await connection_1.pool.query("SELECT order_id FROM orders_in_hand WHERE order_id LIKE ? ORDER BY order_id DESC LIMIT 1", [`${prefix}%`]);
        if (!rows[0]) {
            return `${prefix}00001`;
        }
    ...
    async update(id, data) {
        ...
        if (updates.length > 0) {
            values.push(id);
            await connection_1.pool.query(`UPDATE orders_in_hand SET ${updates.join(", ")} WHERE id = ?`, values);
        }
        return this.findById(id);
    }
    async delete(id) {
        await connection_1.pool.query("DELETE FROM orders_in_hand WHERE id = ?", [id]);
    }
    mapToEntity(row) {
        return {
            id: row.id,
...
exports.MySqlOrderInHandRepository = MySqlOrderInHandRepository;
//# sourceMappingURL=MySqlOrderInHandRepository.js.map