...
exports.KpiFormulaEvaluator = void 0;
...
class KpiFormulaEvaluator {
    ...
    static validate(formula) {
        if (!formula || formula.trim().length === 0) {
            throw new DomainError_1.ValidationError("Formula cannot be empty.");
        }
        if (!this.ALLOWED_PATTERN.test(formula)) {
            throw new DomainError_1.ValidationError("Formula may only contain numbers, target, actual, + - * / ( ) and whitespace.");
        }
        ...
        for (const word of identifierMatches) {
            if (!this.ALLOWED_IDENTIFIERS.has(word.toLowerCase())) {
                throw new DomainError_1.ValidationError(`Unknown term "${word}" in formula. Only "target" and "actual" are allowed.`);
            }
        }
    ...
    static evaluate(formula, target, actual) {
        ...
        function parseTerm() {
            ...
            while (peek() === "*" || peek() === "/") {
                ...
                if (op === "/") {
                    if (rhs === 0)
                        throw new DomainError_1.ValidationError("Formula evaluation resulted in division by zero.");
                    value = value / rhs;
                }
                else {
                    value = value * rhs;
        ...
        function parseFactor() {
            const tok = peek();
            if (tok === undefined)
                throw new DomainError_1.ValidationError("Unexpected end of formula.");
            if (tok === "-") {
                consume();
                return -parseFactor();
            }
            if (tok === "(") {
                ...
                if (consume() !== ")")
                    throw new DomainError_1.ValidationError("Mismatched parentheses in formula.");
                return value;
            }
            if (tok === "target") {
                consume();
            ...
            if (Number.isNaN(num))
                throw new DomainError_1.ValidationError(`Unexpected token "${tok}" in formula.`);
            consume();
            return num;
        }
        const result = parseExpression();
        if (pos !== tokens.length)
            throw new DomainError_1.ValidationError("Unexpected trailing tokens in formula.");
        return result;
    }
...
exports.KpiFormulaEvaluator = KpiFormulaEvaluator;
KpiFormulaEvaluator.ALLOWED_PATTERN = /^[\d\s.+\-*/()a-zA-Z]+$/;
KpiFormulaEvaluator.ALLOWED_IDENTIFIERS = new Set(["target", "actual"]);
//# sourceMappingURL=KpiFormulaEvaluator.js.map