...
async function ensureDepartment(name) {
    const [rows] = await connection_1.pool.query("SELECT id FROM departments WHERE name = ?", [name]);
    if (rows[0])
        return rows[0].id;
    const id = (0, uuid_1.v4)();
    await connection_1.pool.query("INSERT INTO departments (id, name) VALUES (?, ?)", [id, name]);
    return id;
}
async function ensureDesignation(title) {
    const [rows] = await connection_1.pool.query("SELECT id FROM designations WHERE title = ?", [title]);
    if (rows[0])
        return rows[0].id;
    const id = (0, uuid_1.v4)();
    await connection_1.pool.query("INSERT INTO designations (id, title) VALUES (?, ?)", [id, title]);
    return id;
}
async function ensureRoleId(name) {
    const [rows] = await connection_1.pool.query("SELECT id FROM roles WHERE name = ?", [name]);
...
async function ensureDemoUser(opts) {
    const [existingUser] = await connection_1.pool.query("SELECT id FROM users WHERE email = ?", [opts.email]);
    if (existingUser[0]) {
        const [existingEmployee] = await connection_1.pool.query("SELECT id FROM employees WHERE user_id = ?", [existingUser[0].id]);
        if (existingEmployee[0])
            return existingEmployee[0].id;
    }
    const userId = existingUser[0]?.id ?? (0, uuid_1.v4)();
    if (!existingUser[0]) {
        const passwordHash = await bcryptjs_1.default.hash(DEMO_PASSWORD, env_1.env.bcryptSaltRounds);
        await connection_1.pool.query("INSERT INTO users (id, employee_code, email, password_hash, full_name, status) VALUES (?, ?, ?, ?, ?, 'active')", [userId, opts.employeeCode, opts.email, passwordHash, opts.fullName]);
    }
    const employeeId = (0, uuid_1.v4)();
    await connection_1.pool.query(`INSERT INTO employees (id, employee_code, full_name, email, department_id, designation_id, manager_id, user_id, date_of_joining)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURDATE())`, [employeeId, opts.employeeCode, opts.fullName, opts.email, opts.departmentId, opts.designationId, opts.managerEmployeeId, userId]);
...
async function seedDemoUsers() {
    ...
    const [supervisorRows] = await connection_1.pool.query("SELECT e.id, e.user_id FROM employees e JOIN users u ON u.id = e.user_id WHERE u.email = ?", ["supervisor@liinexus.com"]);
    if (supervisorRows[0]) {
        const supervisorRoleId = await ensureRoleId("Supervisor");
        if (supervisorRoleId) {
            await connection_1.pool.query("INSERT IGNORE INTO user_roles (user_id, role_id, scope_type, scope_id) VALUES (?, ?, 'global', '')", [supervisorRows[0].user_id, supervisorRoleId]);
        }
    }
...
async function seedDemoChecklistTemplate() {
    const [rows] = await connection_1.pool.query("SELECT id FROM checklist_templates WHERE title = ?", ["Daily Housekeeping Checklist"]);
    if (rows[0])
        return;
    const templateId = (0, uuid_1.v4)();
    await connection_1.pool.query("INSERT INTO checklist_templates (id, title, description, frequency) VALUES (?, ?, ?, 'daily')", [templateId, "Daily Housekeeping Checklist", "Standard end-of-day housekeeping checklist for factory floor supervisors."]);
    ...
    for (let i = 0; i < items.length; i++) {
        await connection_1.pool.query("INSERT INTO checklist_template_items (id, template_id, label, sort_order) VALUES (?, ?, ?, ?)", [(0, uuid_1.v4)(), templateId, items[i], i]);
    }
    console.log("Demo checklist template seeded: Daily Housekeeping Checklist");
}
async function seedDemoDelegation() {
    const [rows] = await connection_1.pool.query("SELECT id FROM delegated_tasks WHERE title = ?", ["Prepare weekly production summary"]);
    if (rows[0])
        return;
    const [hodRow] = await connection_1.pool.query("SELECT e.id FROM employees e JOIN users u ON u.id = e.user_id WHERE u.email = 'hod@liinexus.com'");
    const [supervisorRow] = await connection_1.pool.query("SELECT e.id FROM employees e JOIN users u ON u.id = e.user_id WHERE u.email = 'supervisor@liinexus.com'");
    if (!hodRow[0] || !supervisorRow[0])
        return;
    const dueDate = new Date();
    dueDate.setDate(dueDate.getDate() + 3);
    await connection_1.pool.query(`INSERT INTO delegated_tasks (id, title, description, assigned_by, assigned_to, due_date, priority)
     VALUES (?, 'Prepare weekly production summary', 'Summarize this week''s output and delays for the executive meeting.', ?, ?, ?, 'medium')`, [(0, uuid_1.v4)(), hodRow[0].id, supervisorRow[0].id, dueDate.toISOString().slice(0, 10)]);
    console.log("Demo delegated task seeded: Prepare weekly production summary");
}
async function seedDemoFactoryEntry() {
    const [rows] = await connection_1.pool.query("SELECT id FROM factory_production_entries WHERE order_reference = ?", ["DEMO-ORD-001"]);
    if (rows[0])
        return;
    ...
    const [existingShift] = await connection_1.pool.query("SELECT id FROM shifts WHERE name = ?", ["General"]);
    let shiftId = existingShift[0]?.id;
    if (!shiftId) {
        shiftId = (0, uuid_1.v4)();
        await connection_1.pool.query("INSERT INTO shifts (id, name, start_time, end_time) VALUES (?, 'General', '09:00:00', '17:00:00')", [shiftId]);
        await connection_1.pool.query("INSERT INTO shifts (id, name, start_time, end_time) VALUES (?, 'Morning', '08:00:00', '16:00:00')", [(0, uuid_1.v4)()]);
        await connection_1.pool.query("INSERT INTO shifts (id, name, start_time, end_time) VALUES (?, 'Night', '00:00:00', '08:00:00')", [(0, uuid_1.v4)()]);
        console.log("Demo shifts seeded: General, Morning, Night");
    }
    const [existingLine] = await connection_1.pool.query("SELECT id FROM production_lines WHERE name = ?", ["Line A"]);
    if (!existingLine[0]) {
        await connection_1.pool.query("INSERT INTO production_lines (id, name, code) VALUES (?, 'Line A', 'LINE-A')", [(0, uuid_1.v4)()]);
        console.log("Demo production line seeded: Line A");
    }
    const [deptRow] = await connection_1.pool.query("SELECT id FROM departments LIMIT 1");
    const [supervisorRow] = await connection_1.pool.query("SELECT e.id FROM employees e JOIN users u ON u.id = e.user_id WHERE u.email = 'supervisor@liinexus.com'");
    if (!deptRow[0] || !supervisorRow[0])
        return;
    await connection_1.pool.query(`INSERT INTO factory_production_entries
     ...
     VALUES (?, CURDATE(), ?, ?, 'DEMO-ORD-001', 'finished_sku', 'DEMO-SKU-001', 100, 92, 15, 'Material delay', ?, ?, 'submitted')`, [(0, uuid_1.v4)(), shiftId, deptRow[0].id, supervisorRow[0].id, supervisorRow[0].id]);
    console.log("Demo factory production entry seeded: DEMO-ORD-001");
}
async function seedDemoCrmLead() {
    const [rows] = await connection_1.pool.query("SELECT id FROM crm_leads WHERE lead_code = ?", ["LEAD-DEMO01"]);
    if (rows[0])
        return;
    const [merchantRow] = await connection_1.pool.query("SELECT e.id FROM employees e JOIN users u ON u.id = e.user_id WHERE u.email = 'merchant@liinexus.com'");
    ...
    await connection_1.pool.query(`INSERT INTO crm_leads
     ...
     VALUES (?, 'LEAD-DEMO01', CURDATE(), 'James Whitfield', 'Whitfield Home Furnishings', 'United Kingdom', 'trade_fair',
       'export', 'Dining Furniture', ?, 'qualification', ?, ?, ?, 'high')`, [(0, uuid_1.v4)(), merchantRow[0]?.id ?? null, forecastAmount, winProbability, (forecastAmount * winProbability) / 100]);
    console.log("Demo CRM lead seeded: LEAD-DEMO01");
}
async function seedDemoKpiEntries() {
    const [defs] = await connection_1.pool.query("SELECT id, frequency FROM kpi_engine_definitions WHERE deleted_at IS NULL");
    for (const def of defs) {
        const periodKey = def.frequency === "quarterly"
            ? `${new Date().getFullYear()}-Q${Math.floor(new Date().getMonth() / 3) + 1}`
            : `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, "0")}`;
        const [existing] = await connection_1.pool.query("SELECT id FROM kpi_engine_entries WHERE kpi_definition_id = ? AND period_key = ?", [def.id, periodKey]);
        if (existing[0])
            continue;
        await connection_1.pool.query(`INSERT INTO kpi_engine_entries (id, kpi_definition_id, period_key, target, actual, computed_score, traffic_light, weightage_used)
       VALUES (?, ?, ?, 100, 88, 88, 'amber', 15)`, [(0, uuid_1.v4)(), def.id, periodKey]);
    }
    if (defs.length > 0)
        console.log(`Demo KPI Engine entries seeded for ${defs.length} definition(s).`);
}
