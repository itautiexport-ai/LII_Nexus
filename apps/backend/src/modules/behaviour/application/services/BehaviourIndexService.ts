...
import { ComponentScore, PeriodType } from "../../domain/entities/Behaviour";
import { dateRangeForPeriod, lastNPeriodKeys } from "./periodUtils";
...
function weightedAverage(scores: { rawScore: number | null; weightUsed: number }[]): number | null {
  const present = scores.filter((s) => s.rawScore !== null) as { rawScore: number; weightUsed: number }[];
 ...
 */
export class BehaviourIndexService {
  constructor(private readonly repo: IBehaviourRepository) {}

  private async onTimeCompletionRate(employeeId: string, from: string, to: string): Promise<number | null> {
    const [rows] = await pool.query<any[]>(
      `SELECT
         ...
         (SELECT COUNT(*) FROM delegated_tasks WHERE assigned_to = ? AND deleted_at IS NULL AND due_date BETWEEN ? AND ? AND base_status = 'completed') as dtDone`,
      [employeeId, from, to, employeeId, from, to, employeeId, from, to, employeeId, from, to]
    );
    const r = rows[0];
  ...
  private async delayStats(employeeId: string, from: string, to: string): Promise<{ frequency: number | null; averageDelay: number | null }> {
    const [rows] = await pool.query<any[]>(
      `SELECT
         COUNT(*) as total,
       ...
       FROM flowchart_tasks WHERE assigned_to = ? AND due_date BETWEEN ? AND ?`,
      [employeeId, from, to]
    );
    const [dtRows] = await pool.query<any[]>(
      `SELECT
         COUNT(*) as total,
         SUM(CASE WHEN due_date < CURDATE() AND base_status != 'completed' THEN 1 ELSE 0 END) as delayedOpen,
         SUM(CASE WHEN base_status = 'completed' AND completed_at IS NOT NULL AND DATE(completed_at) > due_date THEN 1 ELSE 0 END) as delayedCompleted,
         SUM(CASE WHEN base_status = 'completed' AND completed_at IS NOT NULL AND DATE(completed_at) > due_date THEN DATEDIFF(completed_at, due_date) ELSE 0 END) as totalDelayDays
       FROM delegated_tasks WHERE assigned_to = ? AND deleted_at IS NULL AND due_date BETWEEN ? AND ?`,
      [employeeId, from, to]
    );
    const r = rows[0];
    const d = dtRows[0];
    const total = Number(r.total) + Number(d.total);
    if (total === 0) return { frequency: null, averageDelay: null };
  ...
  private async checklistDiscipline(employeeId: string, from: string, to: string): Promise<number | null> {
    const [rows] = await pool.query<any[]>(
    ...
    const r = rows[0];
    if (Number(r.total) === 0 || Number(r.totalItems) === 0) return null;
    return Math.round((Number(r.checkedItems) / Number(r.totalItems)) * 10000) / 100;
  }

  private async followupDiscipline(employeeId: string, from: string, to: string): Promise<number | null> {
    const [rows] = await pool.query<any[]>(
      `SELECT SUM(CASE WHEN clf.on_time = 1 THEN 1 ELSE 0 END) as onTime, COUNT(*) as total
       ...
       WHERE cl.assigned_merchant_id = ? AND clf.completed_at IS NOT NULL AND DATE(clf.completed_at) BETWEEN ? AND ?`,
      [employeeId, from, to]
    );
  ...
  private async crmDiscipline(employeeId: string): Promise<number | null> {
    const [rows] = await pool.query<any[]>(
  ...
  private async attendanceImpact(employeeId: string, periodType: PeriodType, periodKey: string): Promise<number | null> {
    const [rows] = await pool.query<any[]>(
      `SELECT eks.raw_score FROM employee_kpi_scores eks
       JOIN kpi_definitions kd ON kd.id = eks.kpi_definition_id
       WHERE eks.employee_id = ? AND eks.period_type = ? AND eks.period_key = ? AND kd.name = 'Attendance'`,
      [employeeId, periodType, periodKey]
    );
    return rows[0]?.raw_score !== undefined && rows[0]?.raw_score !== null ? Number(rows[0].raw_score) : null;
  }

  private async taskConsistency(employeeId: string, periodType: PeriodType, referenceKey: string): Promise<number | null> {
    ...
    const keys = lastNPeriodKeys("monthly", 3, new Date(`${referenceKey}-01`));
    const rates: number[] = [];
    for (const key of keys) {
      const { from, to } = dateRangeForPeriod("monthly", key);
  ...
  private async managerFeedbackScore(employeeId: string, periodType: PeriodType, periodKey: string): Promise<number | null> {
    const feedback = await this.repo.getManagerFeedback(employeeId, periodType, periodKey);
    return feedback ? feedback.rating * 20 : null;
  }

  async computeIndex(employeeId: string, periodType: PeriodType, periodKey: string) {
    ...
    const weightOf = (key: string) => components.find((c) => c.componentKey === key && c.status === "active")?.weight ?? 0;
    ...
    const [delegationRows] = await pool.query<any[]>(
    ...
    const baseComponents: ComponentScore[] = [
    ...
    const priorKeys = lastNPeriodKeys(periodType, 2, periodType === "monthly" ? new Date(`${periodKey}-01`) : new Date(`${periodKey}-01-01`));
  ...
  async getIndex(employeeId: string, periodType: PeriodType, periodKey: string) {
