...
export class PayrollService {
  async saveDepartmentWeeklyPayroll(data: any[], weekStartStr: string, weekEndStr: string) {
    ...
    try {
      ...
      await connection.query(
        `DELETE FROM department_weekly_payroll WHERE week_start_date = ? AND week_end_date = ?`,
      ...
      for (const row of data) {
        ...
        await connection.query(
          `INSERT INTO department_weekly_payroll 
  ...
  async getDepartmentMonthlyPayroll(month: number, year: number) {
    ...
    const [rows] = await pool.query<any[]>(sql, [month, year, month, year]);
  ...
  async getWeeklyPayroll(startDateStr: string, endDateStr: string) {
    const sql = `
      SELECT 
        e.id as employee_id,
        e.employee_code,
    ...
    const [rows] = await pool.query<any[]>(sql, [startDateStr, endDateStr]);
  ...
  async getMonthlySalarySheet(startDateStr: string, endDateStr: string) {
    const simplifiedSql = `
      SELECT 
        d.name as department_name,
    ...
    const [rows] = await pool.query<any[]>(simplifiedSql, [startDateStr, endDateStr, startDateStr, endDateStr]);
