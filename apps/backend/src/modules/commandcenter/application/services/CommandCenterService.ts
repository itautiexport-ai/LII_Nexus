...
import { periodKeyForNow, lastNPeriodKeys, dateRangeForPeriod } from "../../../scoring/application/services/scoringPeriodUtils";
import { getRangeForWindow } from "../../../officeperf/application/services/periodUtils";
...
function statusOf(score: number | null): "good" | "warning" | "critical" | "unknown" {
...
export class CommandCenterService {
  ...
  private getBusinessHealth(rankedEmployees: Awaited<ReturnType<RankingService["topPerformers"]>>) {
    const periodKey = periodKeyForNow("monthly");
    const scores = rankedEmployees.map((e) => e.overallScore as number);
  ...
  private async getProductionHealth() {
    const periodKey = periodKeyForNow("monthly");
    const { from, to } = dateRangeForPeriod("monthly", periodKey);
  ...
  private async getPeopleHealth(rankedEmployees: Awaited<ReturnType<RankingService["topPerformers"]>>) {
    const [empRows] = await pool.query<any[]>("SELECT COUNT(*) as total FROM employees WHERE deleted_at IS NULL AND status = 'active'");
    const atRisk = rankedEmployees.filter((e) => (e.overallScore as number) < 50);
    return {
  ...
  private async getDepartmentHealth() {
    const periodKey = periodKeyForNow("monthly");
  ...
  private async getOrderHealth() {
    ...
    const [orderRows] = await pool.query<any[]>(
      `SELECT status, COUNT(DISTINCT order_reference) as count FROM factory_production_entries
  ...
  private async getDelayedTasks() {
    const [flowchartRows] = await pool.query<any[]>(
      `SELECT ft.id, w.name as workflow_name, ws.name as stage_name, e.full_name as assignee_name, ft.due_date
    ...
    const [delegationRows] = await pool.query<any[]>(
      `SELECT dt.id, dt.title, e.full_name as assignee_name, dt.due_date, dt.priority
  ...
  private async getDelayedOrders() {
    const [rows] = await pool.query<any[]>(
      `SELECT DISTINCT wr.id, wr.reference, w.name as workflow_name, wr.started_at
  ...
  private async getDelayedProduction() {
    const [rows] = await pool.query<any[]>(
      `SELECT fpe.id, fpe.entry_date, fd.name as department_name, fpe.delay_minutes, fpe.status,
  ...
  private async getCriticalAlerts(peopleHealth: Awaited<ReturnType<CommandCenterService["getPeopleHealth"]>>, departmentHealth: Awaited<ReturnType<CommandCenterService["getDepartmentHealth"]>>, delayedOrders: unknown[], productionHealth: Awaited<ReturnType<CommandCenterService["getProductionHealth"]>>) {
    ...
    for (const dept of departmentHealth) {
      if (dept.status === "critical") alerts.push({ severity: "critical", message: `${dept.departmentName} department averaging ${dept.averageScore}% this month.` });
    }
    if ((delayedOrders as unknown[]).length > 0) {
      alerts.push({ severity: "warning", message: `${(delayedOrders as unknown[]).length} workflow run(s) have overdue stages.` });
    }
  ...
  private async getFactoryHeatMap() {
    const periodKey = periodKeyForNow("monthly");
    const { from, to } = dateRangeForPeriod("monthly", periodKey);
    const [rows] = await pool.query<any[]>(
  ...
  private async getWeeklyTrend() {
    const weeks: { weekStart: string; completionRate: number | null }[] = [];
    for (let i = 5; i >= 0; i--) {
      const ref = new Date();
      ref.setDate(ref.getDate() - i * 7);
      const { from, to } = getRangeForWindow("week", ref);
      const [rows] = await pool.query<any[]>(
        `SELECT
  ...
  private async getMonthlyTrend() {
    const periodKeys = lastNPeriodKeys("monthly", 6);
    const months = [];
    for (const periodKey of periodKeys) {
      const allScored = await rankingService.topPerformers("monthly", periodKey, 100000);
      const scores = allScored.map((e) => e.overallScore as number);
      const average = scores.length > 0 ? Math.round((scores.reduce((s, v) => s + v, 0) / scores.length) * 100) / 100 : null;
