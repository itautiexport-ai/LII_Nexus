import { ChecklistFrequency } from "../../domain/entities/Checklist";
...
export function computeCurrentPeriod(frequency: ChecklistFrequency, referenceDate: Date = new Date()) {
  if (frequency === "daily") {
...
export function getRangeForWindow(window: "today" | "week" | "month", referenceDate: Date = new Date()) {
  if (window === "today") {
    const key = toDateStr(referenceDate);
