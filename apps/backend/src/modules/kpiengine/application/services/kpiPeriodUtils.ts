import { KpiFrequency } from "../../domain/entities/KpiEngine";
...
export function currentPeriodKey(frequency: KpiFrequency, referenceDate: Date = new Date()): string {
  const y = referenceDate.getFullYear();
  if (frequency === "daily") return `${y}-${pad(referenceDate.getMonth() + 1)}-${pad(referenceDate.getDate())}`;
  if (frequency === "weekly") {
    const { year, week } = isoWeek(referenceDate);
    return `${year}-W${pad(week)}`;
  }
  if (frequency === "monthly") return `${y}-${pad(referenceDate.getMonth() + 1)}`;
  if (frequency === "quarterly") return `${y}-Q${Math.floor(referenceDate.getMonth() / 3) + 1}`;
  return `${y}`;
}
...
export function lastNPeriodKeys(frequency: KpiFrequency, n: number, referenceDate: Date = new Date()): string[] {
  const keys: string[] = [];
  for (let i = n - 1; i >= 0; i--) {
