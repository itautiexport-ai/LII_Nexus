...
export class ProductionEmService {
  /**
   ...
   */
  async getProductionEmReport(startDateStr: string, endDateStr: string) {
    ...
    const [rows] = await pool.query<any[]>(sql, [startDateStr, endDateStr, startDateStr, endDateStr]);
    ...
    try {
      ...
      `;
      const [hrRows] = await pool.query<any[]>(hrSql, [
