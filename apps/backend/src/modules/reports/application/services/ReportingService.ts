...
import { ReportFilters, ReportResult, ReportType } from "../../domain/entities/Report";
import { ScoringEngineService } from "../../../scoring/application/services/ScoringEngineService";
import { RankingService } from "../../../scoring/application/services/RankingService";
...
import { MerchantMetricsService } from "../../../crm/application/services/MerchantMetricsService";
...
function defaultRange(filters: ReportFilters): { from: string; to: string } {
...
export class ReportingService {
  ...
  private async employeePerformance(filters: ReportFilters): Promise<ReportResult> {
    const periodKey = periodKeyForNow("monthly");
    ...
    const avg = scored.length > 0 ? Math.round((scored.reduce((s, r) => s + (r.overallScore as number), 0) / scored.length) * 100) / 100 : null;
    return {
  ...
  private async departmentPerformance(filters: ReportFilters): Promise<ReportResult> {
    const periodKey = periodKeyForNow("monthly");
    const departments = await rankingService.departmentRanking("monthly", periodKey);
    const rows = departments.map((d) => [d.departmentName, d.averageScore, d.employeeCount, (d as any).rank]);
    return {
  ...
  private async officePerformance(filters: ReportFilters): Promise<ReportResult> {
  ...
  private async factoryPerformance(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const conditions = ["deleted_at IS NULL", "entry_date BETWEEN ? AND ?"];
    const values: unknown[] = [from, to];
    if (filters.departmentId) { conditions.push("factory_department_id = ?"); values.push(filters.departmentId); }
    const [rows] = await pool.query<any[]>(
      `SELECT fd.name, SUM(fpe.actual_qty) as actual, SUM(fpe.target_qty) as target, SUM(fpe.rejection_qty + fpe.rework_qty) as defects
       FROM factory_production_entries fpe JOIN departments fd ON fd.id = fpe.factory_department_id
       WHERE ${conditions.join(" AND ")} GROUP BY fd.name`,
      values
    );
    ...
    return {
      ...
      chartSeries: rowsOut.map((r) => ({ name: r[0] as string, value: (r[3] as number) ?? 0 })),
    };
  }

  private async workflowReports(filters: ReportFilters): Promise<ReportResult> {
    const [rows] = await pool.query<any[]>(
      `SELECT w.name, wr.status, COUNT(*) as count FROM workflow_runs wr JOIN workflows w ON w.id = wr.workflow_id GROUP BY w.name, wr.status`
    );
    return {
  ...
  private async checklistReports(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const [rows] = await pool.query<any[]>(
      `SELECT ct.title, COUNT(DISTINCT ci.id) as instances,
    ...
    return {
      ...
      chartSeries: rowsOut.map((r) => ({ name: r[0] as string, value: (r[2] as number) ?? 0 })),
    };
  }

  private async delegationReports(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const conditions = ["dt.deleted_at IS NULL", "dt.due_date BETWEEN ? AND ?"];
    const values: unknown[] = [from, to];
    if (filters.employeeId) { conditions.push("dt.assigned_to = ?"); values.push(filters.employeeId); }
    const [rows] = await pool.query<any[]>(
      `SELECT e.full_name, COUNT(*) as total, SUM(CASE WHEN dt.base_status = 'completed' THEN 1 ELSE 0 END) as completed
       FROM delegated_tasks dt JOIN employees e ON e.id = dt.assigned_to
       WHERE ${conditions.join(" AND ")} GROUP BY e.full_name`,
      values
    );
    const rowsOut = rows.map((r) => {
  ...
  private async crmReports(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const conditions = ["deleted_at IS NULL", "inquiry_date BETWEEN ? AND ?"];
    const values: unknown[] = [from, to];
    if (filters.customerName) { conditions.push("contact_name LIKE ?"); values.push(`%${filters.customerName}%`); }
    if (filters.buyerCompany) { conditions.push("company_name LIKE ?"); values.push(`%${filters.buyerCompany}%`); }
    if (filters.status) { conditions.push("status = ?"); values.push(filters.status); }
    const [rows] = await pool.query<any[]>(
      `SELECT status, COUNT(*) as count, SUM(forecast_amount) as forecast FROM crm_leads WHERE ${conditions.join(" AND ")} GROUP BY status`,
      values
    );
    return {
  ...
  private async salesPipeline(filters: ReportFilters): Promise<ReportResult> {
    const conditions = ["deleted_at IS NULL", "status = 'active'"];
    const values: unknown[] = [];
    if (filters.merchantId) { conditions.push("assigned_merchant_id = ?"); values.push(filters.merchantId); }
    const [rows] = await pool.query<any[]>(
      `SELECT sales_stage, COUNT(*) as count, SUM(forecast_amount) as forecast, SUM(weighted_forecast) as weighted
       FROM crm_leads WHERE ${conditions.join(" AND ")} GROUP BY sales_stage`,
      values
    );
    return {
  ...
  private async merchantPerformance(filters: ReportFilters): Promise<ReportResult> {
    const [merchantRows] = await pool.query<any[]>(
      "SELECT DISTINCT assigned_merchant_id, (SELECT full_name FROM employees WHERE id = assigned_merchant_id) as name FROM crm_leads WHERE assigned_merchant_id IS NOT NULL AND deleted_at IS NULL" +
      (filters.merchantId ? " AND assigned_merchant_id = ?" : ""),
    ...
    return {
      ...
      chartSeries: rowsOut.map((r) => ({ name: r[0] as string, value: (r[5] as number) ?? 0 })),
    };
  }

  private async productionReports(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const conditions = ["fpe.deleted_at IS NULL", "fpe.entry_date BETWEEN ? AND ?"];
    const values: unknown[] = [from, to];
    if (filters.departmentId) { conditions.push("fpe.factory_department_id = ?"); values.push(filters.departmentId); }
    if (filters.status) { conditions.push("fpe.status = ?"); values.push(filters.status); }
    const [rows] = await pool.query<any[]>(
      `SELECT fpe.entry_date, fd.name as dept, fpe.actual_qty, fpe.target_qty, fpe.status
       FROM factory_production_entries fpe JOIN departments fd ON fd.id = fpe.factory_department_id
       WHERE ${conditions.join(" AND ")} ORDER BY fpe.entry_date DESC LIMIT 500`,
      values
    );
    return {
  ...
  private async executiveReports(filters: ReportFilters): Promise<ReportResult> {
    const periodKey = periodKeyForNow("monthly");
    const departments = await rankingService.departmentRanking("monthly", periodKey);
  ...
  private async dailyProductionReport(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const conditions = ["de.deleted_at IS NULL", "de.entry_date BETWEEN ? AND ?"];
    const values: unknown[] = [from, to];
    if (filters.departmentId) { conditions.push("de.factory_department_id = ?"); values.push(filters.departmentId); }

    const [rows] = await pool.query<any[]>(
      `SELECT de.entry_date, fd.name as dept, MAX(e.full_name) as supervisor,
  ...
  private async dprProductReport(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const conditions = ["de.deleted_at IS NULL", "de.entry_date BETWEEN ? AND ?"];
    const values: unknown[] = [from, to];
    if (filters.departmentId) { conditions.push("de.factory_department_id = ?"); values.push(filters.departmentId); }
    ...
    const [rows] = await pool.query<any[]>(
      `SELECT de.entry_date, fd.name as dept, u.full_name as supervisor,
  ...
  private async dprDetailedReport(filters: ReportFilters): Promise<ReportResult> {
    const { from, to } = defaultRange(filters);
    const conditions = ["de.deleted_at IS NULL", "de.entry_date BETWEEN ? AND ?"];
    const values: unknown[] = [from, to];
    if (filters.departmentId) { conditions.push("de.factory_department_id = ?"); values.push(filters.departmentId); }
