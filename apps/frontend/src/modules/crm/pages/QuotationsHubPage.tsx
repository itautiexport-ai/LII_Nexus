...
export function QuotationsHubPage() {
  const user = useAuthStore((state: any) => state.user);
  const userRoles = user?.roles || [];
  const isSystemAdmin = userRoles.includes("System Admin");

  const allItems = [
    { label: "Add Quotation", to: "/admin/crm/quotations/new", desc: "Create a new quotation" },
    { label: "List Quotations", to: "/admin/crm/quotations", desc: "View all quotations" },
  ];

  const items = allItems.filter(item => {
    if (isSystemAdmin) return true;
    const itemRole = `Menu: CRM -> Quotations -> ${item.label}`;
    ...
    <div style={{ padding: "32px 40px", maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 16, maxWidth: 600, lineHeight: 1.5 }}>
          Access all quotation tools and features below.
        </p>
      </div>
