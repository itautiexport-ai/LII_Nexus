...
export function ProductionReportHubPage() {
  const user = useAuthStore((state: any) => state.user);
  const userRoles = user?.roles || [];
  const isSystemAdmin = userRoles.includes("System Admin");

  const allItems = [
    { label: "Daily Production Report", to: "/admin/reports/daily-production", desc: "View daily production reports" },
    { label: "Detailed DPR", to: "/admin/reports/detailed-production", desc: "View detailed DPR reports" },
  ];

  const items = allItems.filter(item => {
    if (isSystemAdmin) return true;
    const itemRole = `Menu: Reports -> Production Report -> ${item.label}`;
    ...
    <div style={{ padding: "32px 40px", maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 16, maxWidth: 600, lineHeight: 1.5 }}>
          Access all production reports and insights below.
        </p>
      </div>
