...
export function MachineEfficiencyHubPage() {
  const user = useAuthStore((state: any) => state.user);
  const userRoles = user?.roles || [];
  const isSystemAdmin = userRoles.includes("System Admin");

  const allItems = [
    { label: "Add Efficiency", to: "/admin/machine-efficiency/new", desc: "Add a new machine efficiency record" },
    { label: "List Efficiency", to: "/admin/machine-efficiency", desc: "View all machine efficiency records" },
  ];

  const items = allItems.filter(item => {
    if (isSystemAdmin) return true;
    const itemRole = `Menu: Machine Shop -> Machine Efficiency -> ${item.label}`;
    ...
    <div style={{ padding: "32px 40px", maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ marginBottom: 40 }}>
        <h1 style={{ 
          ...
          Machine Efficiency Module
        </h1>
        <p style={{ color: "#64748b", fontSize: 16, maxWidth: 600, lineHeight: 1.5 }}>
          Access all machine efficiency tools and features below.
        </p>
      </div>
