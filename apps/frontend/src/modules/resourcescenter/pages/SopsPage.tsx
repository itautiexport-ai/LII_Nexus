...
import { documentApi, DocumentRecord, DocumentCategory } from "../../documents/api/documentApi";
import { departmentsApi, DepartmentRecord } from "../../admin/organization/departments/api/departmentsApi";

type Tab = "upload" | "list";

const SOP_CATEGORIES: Record<string, string> = {
  sop: "SOP",
  work_instruction: "Work Flow Instruction",
};
...
export default function SopsPage() {
  ...
  async function loadDocs() {
    setLoading(true);
    try {
      const categoriesToLoad = filterCategory ? [filterCategory] : ["sop", "work_instruction"];
      const res = await documentApi.list({ search: search || undefined });
      
      const filtered = res.items.filter(doc => 
        categoriesToLoad.includes(doc.category) && 
  ...
  }, [tab, search, filterCategory, filterDepartment]);
    ...
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#111827", margin: 0 }}>
          SOPs & Work Flow Instructions
        </h1>
        <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4 }}>
          Upload and manage Standard Operating Procedures and Work Flow Instructions.
        </p>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: "2px solid #e5e7eb", marginBottom: 28, gap: 0 }}>
        {(["upload", "list"] as Tab[]).map((t) => {
          const labels: Record<Tab, string> = { upload: "📤  Upload Document", list: "📋  List of Documents" };
          const active = tab === t;
          return (
            <button
              key={t}
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Assembly Line SOP"
                style={inputStyle}
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                style={inputStyle}
              >
                {Object.entries(SOP_CATEGORIES).map(([k, v]) => (
              ...
              >
                {file ? (
                  ...
                  <>
                    ...
                    <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 4 }}>PDF, DOCX, XLSX, PNG, JPG, MP4 etc.</div>
                  </>
                )}
              </div>
              <input
      ...
      {tab === "list" && (
        <div>
          ...
          {/* Table */}
          {loading ? (
            <div style={{ textAlign: "center", padding: "40px 0", color: "#9ca3af", fontSize: 14 }}>Loading documents…</div>
          ) : docs.length === 0 ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13.5 }}>
                <thead>
                  <tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
                    {["#", "Title", "Attachment", "Type", "Department", "Last Updated", "Actions"].map((h) => (
                  ...
                  {docs.map((doc, i) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "11px 14px", color: "#6b7280" }}>
                        {SOP_CATEGORIES[doc.category] ?? doc.category}
                      </td>
                      <td style={{ padding: "11px 14px", color: "#6b7280" }}>
