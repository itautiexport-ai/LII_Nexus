    ...
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4 }}>
          Upload and manage reusable company templates.
        </p>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: "2px solid #e5e7eb", marginBottom: 28, gap: 0 }}>
        {(["upload", "list"] as Tab[]).map((t) => {
          const labels: Record<Tab, string> = { upload: "📤  Upload Template", list: "📋  List of Templates" };
          const active = tab === t;
          return (
            <button
              key={t}
      ...
      {tab === "upload" && (
        <div style={{ maxWidth: 560 }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* Title */}
            <div>
              <label style={labelStyle}>Template Title <span style={{ color: "#ef4444" }}>*</span></label>
              <input
                required
                value={form.title}
      ...
      {tab === "list" && (
        <div>
          ...
          {/* Table */}
          {loading ? (
            <div style={{ textAlign: "center", padding: "40px 0", color: "#9ca3af", fontSize: 14 }}>Loading templates…</div>
          ) : docs.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px 0", color: "#9ca3af" }}>
              <div style={{ fontSize: 36, marginBottom: 12 }}>📂</div>
              <div style={{ fontSize: 14 }}>No templates found.</div>
              <button
